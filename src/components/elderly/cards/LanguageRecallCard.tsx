...
interface LanguageRecallCardProps {
  activity: FeedActivity;
}

export default function LanguageRecallCard({ activity }: LanguageRecallCardProps) {
  const {
    language,
    recordAttempt,
    ...
    <div className="bg-stone-900/90 text-stone-100 rounded-3xl p-6 sm:p-8 shadow-2xl border border-stone-800 backdrop-blur-sm max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <span className="px-3 py-1 rounded-full text-xs font-semibold bg-amber-500/20 text-amber-300 border border-amber-500/30">
          Language & Semantic Naming • Assam
        </span>
        <button
      ...
      {/* Decorative Red-and-White Phulam Gamosa Border Motif */}
      <div className="h-2 w-full bg-gradient-to-r from-red-600 via-white to-red-600 rounded-full mb-6 opacity-80" />
      ...
      <DignifiedFeedback
        ...
        subMessage={
          isCorrectFeedback
            ? 'The Phulam Gamosa is woven on household looms with deep affection.'
