...
import { Volume2, Sun, Moon, Sunrise, CheckCircle2 } from 'lucide-react';
...
export default function OrientationCard({ activity }: OrientationCardProps) {
  const {
    language,
    recordAttempt,
    ...
    <div className="bg-stone-900/90 text-stone-100 rounded-3xl p-6 sm:p-8 shadow-2xl border border-stone-800 backdrop-blur-sm max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <span className="px-3 py-1 rounded-full text-xs font-semibold bg-sky-500/20 text-sky-300 border border-sky-500/30">
          Environmental Circadian Orientation
        </span>
        <button
      ...
      <div className="flex justify-center mb-4">
        <div className="w-16 h-16 rounded-full bg-amber-500/20 border border-amber-500/40 flex items-center justify-center">
          <Sunrise className="w-8 h-8 text-amber-400 animate-pulse" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {activity.options?.map((opt) => {
          ...
          const isMorning = opt.id.includes('morning');
            ...
            <button
              ...
              className={`p-4 rounded-2xl flex flex-col items-center justify-center text-center transition-all border-2 ${
                isSelected
                  ...
                  : 'bg-stone-800/90 border-stone-700 hover:border-amber-500 text-stone-200'
              }`}
            >
              <div className="mb-2">
                {isMorning ? (
                  <Sunrise className="w-6 h-6 text-amber-400" />
                ) : opt.id.includes('noon') ? (
                  <Sun className="w-6 h-6 text-yellow-500" />
                ) : (
                  <Moon className="w-6 h-6 text-indigo-400" />
                )}
              </div>
              <span className="font-semibold text-sm sm:text-base">{optText}</span>
            </button>
          );
        })}
      </div>
      ...
      <DignifiedFeedback
        ...
        message={isCorrectFeedback ? activity.dignifiedPraise : 'Good morning, Aita ❤️'}
        subMessage={
          isCorrectFeedback
            ? 'The morning light brings clarity and warmth.'
